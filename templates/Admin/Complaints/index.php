		...
		<div class="dropdown mx-3 mt-2">
			...
			<i class="fa-solid fa-bars text-primary"></i>
				...
				<div class="dropdown-menu dropdown-menu-end" aria-labelledby="orederStatistics">
							<li><?= $this->Html->link(__('<i class="fa-solid fa-plus"></i> New Complaint'), ['action' => 'add'], ['class' => 'dropdown-item', 'escapeTitle' => false]) ?></li>
							</div>
		</div>
</div>
</div>
	...
	<div class="table-responsive">
		...
		?>
                <tr>
                    ...
                    <th><?= $this->Paginator->sort('hostel_id') ?></th>
                    <th><?= $this->Paginator->sort('room_id') ?></th>
                    <th><?= $this->Paginator->sort('complaintaction_id') ?></th>
                    <th><?= $this->Paginator->sort('complaintcategory_id') ?></th>
            ...
            <tbody>
				...
				<td><?php echo $counter++ ?></td>
                    ...
                    <td><?= $complaint->hasValue('hostel') ? $this->Html->link($complaint->hostel->hostel_id, ['controller' => 'Hostels', 'action' => 'view', $complaint->hostel->id]) : '' ?></td>
                    <td><?= $complaint->hasValue('room') ? $this->Html->link($complaint->room->hostel_id, ['controller' => 'Rooms', 'action' => 'view', $complaint->room->id]) : '' ?></td>
                    <td><?= h($complaint->complaintaction_id) ?></td>
                    <td><?= h($complaint->complaintcategory_id) ?></td>
                    <td><?= $this->Number->format($complaint->status) ?></td>
                    <td><?= h($complaint->created) ?></td>
                    <td><?= h($complaint->modified) ?></td>
					<td class="actions text-center">
						<div class="btn-group shadow" role="group" aria-label="Basic example">
							<?= $this->Html->link(__('<i class="far fa-folder-open"></i>'), ['action' => 'view', $complaint->id], ['class' => 'btn btn-outline-primary btn-xs', 'escapeTitle' => false]) ?>
							<?= $this->Html->link(__('<i class="fa-regular fa-pen-to-square"></i>'), ['action' => 'edit', $complaint->id], ['class' => 'btn btn-outline-warning btn-xs', 'escapeTitle' => false]) ?>
							...
							<?= $this->Form->postLink(
								...
								[
									'confirm' => __('Are you sure you want to delete Complaints: "{0}"?', $complaint->id),
									'title' => __('Delete'),
...
const status = new Chart(ctx_2, {
    ...
    data: {
        ...
        datasets: [{
            label: '# of Complaints(s)',
			data: [<?= json_encode($total_complaints_active); ?>, <?= json_encode($total_complaints_disabled); ?>, <?= json_encode($total_complaints_archived); ?>],
            backgroundColor: [
...
<div class="card bg-body-tertiary border-0 shadow mb-4">
	<div class="card-body">
		<div class="card-title mb-0">Search</div>
			<div class="tricolor_line mb-3"></div>
			<?php echo $this->Form->create(null, ['valueSources' => 'query', 'url' => ['controller' => 'Complaints','action' => 'index']]); ?>
				<fieldset>
